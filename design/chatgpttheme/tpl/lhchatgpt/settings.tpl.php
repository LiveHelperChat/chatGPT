...
<form action="" method="post" ng-non-bindable>
    ...
    <div class="btn-group" role="group" aria-label="Basic example">
        ...
        <button name="CreateUpdateRestAPI" class="btn btn-sm btn-info"><?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('messagebird/module','Create/Update Rest API/Bot'); ?></button>

        <?php if (\erLhcoreClassModelGenericBotRestAPI::getCount(['filter' => ['name' => 'ChatGPTAssistant']]) == 1) : ?>
        <button name="RemoveRestAPI" class="btn btn-sm btn-danger"><?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('messagebird/module','Remove Rest API/Bot call'); ?></button>
        <?php endif; ?>
    </div>
