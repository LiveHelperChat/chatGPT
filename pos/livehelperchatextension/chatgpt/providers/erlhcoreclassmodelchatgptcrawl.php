...
foreach (['crawl_frequency','last_crawled_at','status', 'number_of_pages', 'max_pages', 'lhc_file_id'] as $posAttr) {
...
foreach (['name','vector_storage_id', 'url', 'file_id', 'base_url', 'start_url'] as $posAttr) {
    $def->properties[$posAttr] = new ezcPersistentObjectProperty();
    $def->properties[$posAttr]->columnName   = $posAttr;
    $def->properties[$posAttr]->propertyName = $posAttr;
